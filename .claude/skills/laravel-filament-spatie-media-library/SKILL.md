...
- Each media collection generates exactly 2 primary conversions unless a justified variation exists:
  - `optimized` = cropped/resized in original format
  - `web` = cropped/resized in WebP format
...
This conversion is the resized/cropped fallback in the original format.
...
This gives a consistent semantic structure:
...
- media often needs to be visible in the admin preview and public output
...
Use `SpatieMediaLibraryFileUpload` with:

- collection
- conversion
- helper text from constants
- validation from constants
- JPEG/PNG restriction
- public visibility
...
- storing image paths in normal model columns for images that belong in media collections
- hardcoding image dimensions in multiple layers
- omitting model constants
- using `format()` on `optimized`
- using different dimensions for `optimized` and `web` for the same image size
- allowing unsupported upload formats by default
- omitting `visibility('public')`
- omitting `nonQueued()`
- using multiple files in a collection without a clear requirement
- building ad hoc upload logic outside Spatie Media Library
- duplicating seed media attachments on every seeder run
- exposing inconsistent API key names for media URLs
- using guessed conversion names in Filament tables or forms
